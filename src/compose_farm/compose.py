...
# Regex to extract host from Traefik Host() rule
_HOST_RULE_PATTERN = re.compile(r"Host\(`([^`]+)`\)")
_MIN_ROUTER_LABEL_PARTS = 4


def _extract_routers_from_labels(
    labels: dict[str, str],
) -> dict[str, dict[str, str]]:
    """Extract router configurations from Traefik labels."""
    ...
    for key, value in labels.items():
        ...
        # Parse: traefik.http.routers.{name}.{field}
        parts = key.split(".")
        if len(parts) < _MIN_ROUTER_LABEL_PARTS:
            continue

        router_name = parts[3]
        field = ".".join(parts[4:]) if len(parts) > _MIN_ROUTER_LABEL_PARTS else ""

        if router_name not in routers:
            routers[router_name] = {}

        if field == "rule":
            routers[router_name]["rule"] = value
        elif field == "entrypoints":
            routers[router_name]["entrypoints"] = value

    return routers


def _extract_urls_from_routers(
    routers: dict[str, dict[str, str]],
    urls: dict[str, str],
) -> None:
    """Extract URLs from router configurations, updating the urls dict in place."""
    for router_info in routers.values():
        ...
        scheme = "https" if "websecure" in entrypoints else "http"

        # Extract host(s) from rule
        for match in _HOST_RULE_PATTERN.finditer(rule):
            host = match.group(1)
...
def extract_website_urls(config: Config, stack: str) -> list[str]:
    ...
    Parses traefik.http.routers.*.rule labels for Host() rules and returns
    URLs based on the entrypoint (https for websecure, http for web).
    ...
    for definition in raw_services.values():
        ...
        # Skip if traefik is disabled
        enable_raw = labels.get("traefik.enable")
        if enable_raw is not None and enable_raw.lower() == "false":
            continue

        routers = _extract_routers_from_labels(labels)
        _extract_urls_from_routers(routers, urls)
